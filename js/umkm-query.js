    ...
    .then(datas => {
        datas.forEach(data => {
            let card = `
                        ...
                        <a href="view?id=${data.id}" target="_self"><div style="background-image: url('assets/umkm/${data.gambar}');" class="img-product ${data.gambar == '' ? 'no-img-product' : ''}">${data.gambar == '' ? 'gambar tidak tersedia :(' : ''}</div></a>
